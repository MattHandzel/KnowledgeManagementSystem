...
class CaptureUI:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.keybindings: Optional[SemanticKeybindings] = None
    ...
    def draw_ui(self):
        ...
        self.stdscr.addstr(
            tags_y, 2, "Tags:", curses.color_pair(1) if tags_active else 0
        )
        self.draw_tags()
    ...
    def draw_field_border(
        self, y: int, x: int, height: int, width: int, title: str, active: bool
    ):
    ...
    def draw_context(self):
        ...
        display_text = (
            self.context[: win_width - 1]
            if len(self.context) >= win_width
            else self.context
        )
    ...
    def draw_tags(self):
        ...
        display_text = (
            self.tags[: win_width - 1] if len(self.tags) >= win_width else self.tags
        )
    ...
    def draw_sources(self):
        ...
        self.sources_win.addstr(
            0, 0, self.sources[: self.sources_win.getmaxyx()[1] - 1]
        )
    ...
    def draw_idea_list(self):
        ...
        if not self.idea_files:
            self.idea_list_win.addstr(
                1, 1, "No ideas found. Press ESC to return to capture mode."
            )
        else:
    ...
    def position_cursor(self):
        ...
        if self.active_field == Field.CONTENT:
            ...
            if 0 <= cursor_line < self.content_win.getmaxyx()[0]:
                self.content_win.move(
                    cursor_line,
                    min(cursor_col, self.content_win.getmaxyx()[1] - 1),
                )
    ...
    def _run_ui_loop(self, stdscr) -> bool:
        ...
        while True:
            ...
            try:
                ...
                assert self.keybindings is not None
    ...
    def insert_tags_char(self, char: str):
        ...
        self.tags = self.tags[: self.tags_cursor] + char + self.tags[self.tags_cursor :]
        self.tags_cursor += 1
    ...
    def delete_tags_char_before(self):
        ...
        if self.tags_cursor > 0:
            self.tags = (
                self.tags[: self.tags_cursor - 1] + self.tags[self.tags_cursor :]
            )
            self.tags_cursor -= 1
    ...
    def delete_tags_char_after(self):
        ...
        if self.tags_cursor < len(self.tags):
            self.tags = (
                self.tags[: self.tags_cursor] + self.tags[self.tags_cursor + 1 :]
            )
    ...
    def capture_clipboard_content(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["wl-paste", "-l"], capture_output=True, text=True, timeout=2
            )
    ...
    def save_capture(self):
        ...
        try:
            ...
            capture_data = {
                ...
                "context": (
                    {"activity": self.context.strip()} if self.context.strip() else {}
                ),
    ...
    def next_idea(self):
        ...
        if self.idea_files:
            self.selected_idea_index = min(
                len(self.idea_files) - 1, self.selected_idea_index + 1
            )
